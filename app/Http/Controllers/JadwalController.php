...
class JadwalController extends Controller
{
    public function indexKelola()
    {
        return view('admin.jadwal.kelola.index-kelola', [
            ...
            // 'hari' => Hari::where('id', $idHari)->first(),
    ...
    {
        for ($i = 1; $i < 3; $i++) {
            Jadwal::where('id_populasi', $i + 4)->delete();
            Populasi::where('id', $i + 4)->delete();
            // Lakukan Mutasi Pada Child 1 & 2
            Populasi::create([
                'id' => $i + 4,
                'fitness' => 0,
            ]);
    ...
    public function popTermination()
    {
        ...
        $worstId = $sortedPop->take(2)->pluck('id')->toArray();
        Jadwal::where('id_populasi', $worstId[0])->delete();
        Jadwal::where('id_populasi', $worstId[1])->delete();
        Populasi::where('id', $worstId[0])->delete();
        Populasi::where('id', $worstId[1])->delete();

        $worstInstance1 = Populasi::find($worstId[0]);
        $worstInstance2 = Populasi::find($worstId[1]);

        if ($worstInstance1) {
            $worstInstance1->delete();
        }

        if ($worstInstance2) {
            $worstInstance2->delete();
        }
